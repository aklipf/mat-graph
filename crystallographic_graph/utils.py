...
class shape:
    ...
    def assert_match(
        self, x: torch.Tensor, context: Dict[str, int] = {}, device: str = None
    ):
        ...
        if device is not None:
            assert x.device == device, f"device ({x.device}) should match with {device}"
...
def assert_tensor_match(
    *args: Tuple[torch.Tensor, shape]
    ...
    for x, s in args:
        context = s.assert_match(x, context=context, device=device)
